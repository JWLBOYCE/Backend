...
app.setErrorHandler((err, req, reply) => {
    ...
    reply.status(status).send({ statusCode: status, error: status === 500 ? 'Internal Server Error' : 'Error', message: status === 500 ? 'An unexpected error occurred' : err?.message || 'Error' });
});
