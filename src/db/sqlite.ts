...
export function getDB() {
  ...
  try { cached.pragma('journal_mode = WAL'); cached.pragma('synchronous = NORMAL'); } catch {}
  return cached;
}

export function detectSchema(db: any) {
  const hasTable = (name: string) => !!db.prepare("SELECT 1 FROM sqlite_master WHERE type='table' AND name=? LIMIT 1").get(name);
  const cols = db.prepare('PRAGMA table_info(posts)').all();
  const hasCol = (c: string) => cols.some((r: any) => String(r.name).toLowerCase() === c.toLowerCase());
  return {
