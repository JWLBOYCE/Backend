...
export default async function routes(f: FastifyInstance) {
  ...
  f.get('/analytics', async (_req, reply) => {
    ...
    const last7 = db.prepare("SELECT COUNT(*) AS c FROM posts WHERE datetime(created_at) >= datetime('now','-7 days')").get().c as number;
    const last30 = db.prepare("SELECT COUNT(*) AS c FROM posts WHERE datetime(created_at) >= datetime('now','-30 days')").get().c as number;
  ...
  f.get('/facets', async (_req, reply) => {
    ...
    for (const r of rows) {
      const cls = String((r as any).c || '');
      let base = cls; const parts = cls.split(' - '); if (parts.length > 1) base = parts[1];
      result[base] = (result[base] || 0) + (r as any).cnt;
    }
