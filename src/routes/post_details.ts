...
export default async function routes(f: FastifyInstance) {
  ...
  f.get('/posts/:id/comments', async (req, reply) => {
    ...
    if (!res) return reply.send({ rows: [] });
    const p = idParam.safeParse(req.params as any);
    if (!p.success) { reply.code(400).send({ error: 'Invalid id' }); return; }
    const id = p.data.id;
  ...
  f.get('/posts/:id/transcript', async (req, reply) => {
    ...
    reply.send({ transcript: (row as any)?.video_transcript ?? null });
  });
}
